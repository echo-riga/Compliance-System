            ...
            if (isset($_POST['register'])) {
              ...
              //  $type = $_POST['type'];
                ...
                if ($stmt->rowcount() > 0) {
                    ...
                    $sql = "INSERT INTO users (employeeid,firstname,lastname,middlename,email,password,passwordtxt,passphrase,status_of_employment) VALUES (?,?,?,?,?,?,?,?,?)";
                    $stmt = $this->conn()->prepare($sql);
                    $stmt->execute([$employeeid,$firstname,$lastname,$middlename,$email,$password,$passwordtxt,$passphrase,$status_of_employment]);
